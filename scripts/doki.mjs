...
const RUNTIME_RELEASE = "runtime-bases-v1";
...
function bootstrapRuntime() {
  ...
  for (const file of ["jetbrains-base.zip", "hyper-base.zip"]) {
    const target = path.join(ROOT, "vendor", file);
    if (fs.existsSync(target)) {
      console.log(`${file}: ya existe`);
      continue;
    }
    const url = `https://github.com/${REPOSITORY}/releases/download/${RUNTIME_RELEASE}/${file}`;
    run("curl", ["-fL", url, "-o", target]);
  }
}
...
function buildHyper(themes = validateThemes()) {
  ...
  if (!fs.readFileSync(indexPath, "utf8").includes("NiniCustomizations")) {
    throw new Error("El Hyper base no contiene el punto de extension NiniCustomizations");
  }
  const packagePath = path.join(pluginRoot, "package.json");
  const packageJson = readJson(packagePath);
  packageJson.version = HYPER_VERSION;
  packageJson.description = "Doki Theme for Hyper generated by Nini Doki Workshop";
...
function exportPortable(themes) {
  ...
  for (const file of fs.readdirSync(DIST_DIR).filter((name) => name.endsWith(".zip"))) {
    fs.copyFileSync(path.join(DIST_DIR, file), path.join(backups, file));
  }
  console.log(`Proyecto portable: ${portable}`);
}

function usage() {
  console.log(`Nini Doki Workshop
